...
window.GMFleetUsers = (() => {
 ...
 async function invoke(name,body){const {data,error}=await ctx.db.functions.invoke(name,{body});if(error){let message=error.message;try{message=(await error.context.json()).error||message;}catch{}throw new Error(message);}if(data?.error)throw new Error(data.error);return data;}
 ...
 function edit(account){
  ...
  const d=dialog(account?'Modifier l’utilisateur':'Ajouter un utilisateur',`<form id="userForm"><div class="fields"><label>Nom complet<input name="display_name" required maxlength="200" value="${e(account?.display_name||'')}"></label><label>E-mail<input name="email" type="email" required maxlength="254" value="${e(account?.pending_email||account?.email||'')}"></label><label>Rôle<select name="role" ${self?'disabled':''}>${options(account?.role||'agent')}</select></label>${account?`<label>Accès<select name="active" ${self?'disabled':''}><option value="true" ${account.active?'selected':''}>Actif</option><option value="false" ${!account.active?'selected':''}>Désactivé</option></select></label>`:''}</div><p class="muted">Agent : suivi des dossiers et contrats. Caissier : dossiers et encaissements. Administrateur : caisse et annulation de contrats. Super Admin : tous les droits, y compris la gestion des utilisateurs.</p>${account?'<p class="muted">L’adresse e-mail sert à la connexion. Après correction d’une invitation, générez un nouveau lien pour la bonne adresse.</p>':''}<div class="form-error" role="alert"></div><div class="form-footer"><button type="submit" class="primary">${account?'Enregistrer':'Créer le lien d’invitation'}</button></div></form>`);
  $('userForm').onsubmit=async event=>{
   ...
   try{
    if(account){
     ...
     if(email!==account.email?.toLowerCase()||account.pending_email){const result=await invoke('staff-email',{user_id:account.user_id,revision:account.revision,email});account.email=result.email;account.revision=result.revision;account.pending_email=null;}
     await rpc('manage_staff',{p:{user_id:account.user_id,revision:account.revision,display_name:f.get('display_name'),role:self?'super_admin':f.get('role'),active:self||f.get('active')==='true'}});d.close();await render(ctx);ctx.notify('Accès utilisateur enregistré.');}
    else await invite({display_name:f.get('display_name'),email:f.get('email'),role:f.get('role')});
   }catch(error){if(account){try{const latest=(await rpc('list_staff_accounts')).find(a=>a.user_id===account.user_id);if(latest)Object.assign(account,latest);}catch{}}form.querySelector('.form-error').textContent=error.message+' Vérifiez les informations puis réessayez. Les changements e-mail déjà confirmés sont conservés.';}finally{b.disabled=false;}
  };
 }
 function remove(account){
  const d=dialog('Supprimer l’utilisateur',`<p>Supprimer <strong>${ctx.escape(account.display_name)}</strong> (${ctx.escape(account.email)}) ?</p><p>Ce compte disparaîtra de la liste et perdra immédiatement l’accès aux opérations. Son identité et ses actions passées restent conservées pour l’historique. Cette suppression de l’accès ne détruit pas le compte d’authentification.</p><form id="deleteUserForm"><label>Saisissez SUPPRIMER pour confirmer<input name="confirmation" required autocomplete="off" pattern="SUPPRIMER"></label><div class="form-error" role="alert"></div><div class="form-footer"><button class="secondary" type="button" data-close="userDialog">Annuler</button><button class="primary" type="submit">Supprimer l’utilisateur</button></div></form>`);
  $('deleteUserForm').onsubmit=async event=>{event.preventDefault();const form=event.currentTarget,b=form.querySelector('[type=submit]');if(new FormData(form).get('confirmation')!=='SUPPRIMER')return;b.disabled=true;try{await rpc('delete_staff',{p:{user_id:account.user_id,revision:account.revision}});d.close();await render(ctx);ctx.notify('Utilisateur supprimé de l’équipe. Son accès est révoqué.');}catch(error){form.querySelector('.form-error').textContent=error.message;}finally{b.disabled=false;}};
 }
