...
function fixture(){
 const data={contract_id:'contract',driver_name:'Éric Test',currency:'USD',vehicle:{model:'Toyota Corolla',plate:'1234AB/01'},daily_amount:10,due:10,total_remaining:100,attempts:[],receipts:[]};window.calls=[];
...
(async()=>{fs.mkdirSync(artifacts,{recursive:true});const server=http.createServer((req,res)=>{const file=path.join(root,new URL(req.url,'http://localhost').pathname);try{res.setHeader('Content-Type',file.endsWith('.js')?'text/javascript':file.endsWith('.css')?'text/css':'text/html');res.end(fs.readFileSync(file));}catch{res.writeHead(404);res.end();}});await new Promise(r=>server.listen(0,'127.0.0.1',r));const browser=await chromium.launch({headless:true,channel:'msedge'});try{
 ...
 await page.locator('[name=plate]').fill('1234ab01');assert.equal(await page.locator('[name=plate]').inputValue(),'1234AB/01');assert.equal(await page.locator('[name=phone]').getAttribute('maxlength'),'9');await page.getByRole('button',{name:'Voir mes versements'}).click();await page.getByText('Véhicule : Toyota Corolla',{exact:false}).waitFor();assert.match(await page.locator('#feeSummary').innerText(),/10,30/);
 await page.locator('[name=wallet]').fill('810000000');await page.locator('[name=amount]').fill('10');await page.getByRole('button',{name:'Payer maintenant'}).click();await page.locator('#checkPayment').waitFor();assert.equal(await page.locator('#driverPayForm').count(),0);assert.equal(await page.locator('[data-download-receipt]').count(),0);
