...
function fixture(){
 const data={contract_id:'contract',driver_name:'Éric Test',currency:'USD',vehicle:{model:'Toyota Corolla',plate:'TEST123'},daily_amount:10,due:10,total_remaining:100,attempts:[],receipts:[]};window.calls=[];
 ...
 window.supabase={createClient:()=>({functions:{invoke:async(name,{body})=>{
  ...
  if(body.action==='driver-lookup')return body.phone==='0812345678'&&body.plate==='TEST123'?{data:{token:'b'.repeat(64)}}:{data:{error:'Téléphone ou plaque incorrects'}};
...
(async()=>{fs.mkdirSync(artifacts,{recursive:true});const server=http.createServer((req,res)=>{const file=path.join(root,new URL(req.url,'http://localhost').pathname);try{res.setHeader('Content-Type',file.endsWith('.js')?'text/javascript':file.endsWith('.css')?'text/css':'text/html');res.end(fs.readFileSync(file));}catch{res.writeHead(404);res.end();}});await new Promise(r=>server.listen(0,'127.0.0.1',r));const browser=await chromium.launch({headless:true,channel:'msedge'});try{
 ...
 await page.locator('[name=phone]').fill('0812345678');await page.locator('[name=plate]').fill('WRONG');await page.getByRole('button',{name:'Voir mes versements'}).click();await page.getByText('Téléphone ou plaque incorrects',{exact:true}).waitFor();assert.equal(await page.locator('#driverPayForm').count(),0);
 await page.locator('[name=plate]').fill('TEST123');await page.getByRole('button',{name:'Voir mes versements'}).click();await page.getByText('Véhicule : Toyota Corolla',{exact:false}).waitFor();assert.match(await page.locator('#feeSummary').innerText(),/10,30/);
 await page.locator('[name=wallet]').fill('+243810000000');await page.locator('[name=amount]').fill('10');await page.getByRole('button',{name:'Payer maintenant'}).click();await page.locator('#checkPayment').waitFor();assert.equal(await page.locator('#driverPayForm').count(),0);assert.equal(await page.locator('[data-download-receipt]').count(),0);
