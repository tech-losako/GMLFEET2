...
function fixture(){
 ...
 window.supabase={createClient:()=>({functions:{invoke:async(name,{body})=>{
  ...
  if(body.action==='check'&&window.checkError)return {data:{error:'Vérification Araka indisponible (HTTP 404, code absent)'}};
...
(async()=>{fs.mkdirSync(artifacts,{recursive:true});const server=http.createServer((req,res)=>{const file=path.join(root,new URL(req.url,'http://localhost').pathname);try{res.setHeader('Content-Type',file.endsWith('.js')?'text/javascript':file.endsWith('.css')?'text/css':'text/html');res.end(fs.readFileSync(file));}catch{res.writeHead(404);res.end();}});await new Promise(r=>server.listen(0,'127.0.0.1',r));const browser=await chromium.launch({headless:true,channel:'msedge'});try{
 const page=await browser.newPage({viewport:{width:390,height:844}}),errors=[];page.on('pageerror',e=>errors.push(e.message));await page.route('**/*supabase.co/**',r=>r.abort());await page.route(/supabase-js@|vendor\/supabase-2\.57\.4\.js/,r=>r.fulfill({contentType:'text/javascript',body:'('+fixture.toString()+')();'}));const base='http://127.0.0.1:'+server.address().port;
 ...
 assert.equal(await page.locator('#paymentStatus').isVisible(),false);assert.deepEqual(await page.getByRole('tab').allTextContents(),['Paiement','Mes reçus','Transactions']);
 await page.locator('[name=wallet]').fill('810000000');await page.getByRole('tab',{name:'Transactions',exact:true}).click();assert.equal(await page.locator('#driverPayForm').isVisible(),false);await page.keyboard.press('Home');assert.equal(await page.getByRole('tab',{name:'Paiement',exact:true}).getAttribute('aria-selected'),'true');assert.equal(await page.locator('[name=wallet]').inputValue(),'810000000');
 await page.screenshot({path:path.join(artifacts,'driver-payment-tabs-mobile.png'),fullPage:true});
 ...
 await page.locator('#checkPayment').click();await page.getByText('Paiement confirmé',{exact:true}).waitFor();assert.equal(await page.locator('#panel-history').isVisible(),false);assert.equal(await page.locator('#panel-receipts').isVisible(),false);
 await page.getByRole('tab',{name:'Transactions',exact:true}).click();await page.getByText('Reçu #123',{exact:false}).waitFor();assert.equal(await page.locator('#panel-pay').isVisible(),false);
 await page.getByRole('tab',{name:'Mes reçus',exact:true}).click();
 ...
 await page.locator('#paymentStatus[data-state="pending"]').waitFor();await page.evaluate(()=>window.checkError=true);await page.locator('#checkPayment').click();await page.locator('#paymentStatus[data-state="unknown"]').waitFor();assert.doesNotMatch(await page.locator('body').innerText(),/Araka|HTTP 404|code absent/i);assert.equal(await page.locator('#driverPayForm').count(),0);
 ...
 await page.getByRole('tab',{name:'Transactions',exact:true}).click();await page.screenshot({path:path.join(artifacts,'driver-history-tabs-mobile.png'),fullPage:true});
 await page.setViewportSize({width:1280,height:900});await page.getByRole('tab',{name:'Paiement',exact:true}).click();await page.screenshot({path:path.join(artifacts,'driver-payment-tabs-desktop.png'),fullPage:true});
