...
async function run({action='check',providerData={transactionId:'TX-1',statusCode:200,statusDescription:'APPROVED'},dispatch=true,sandbox=false,invalidCallback=false,transactionId='TX-1',recent=false,identity=null,lookupResult={ok:true}}={}){
 ...
 const db={auth:{getUser:async()=>({data:{user:identity},error:identity?null:{message:'invalid'}})},rpc:async(name,{p}={})=>{if(name==='issue_driver_plate_access'){updates.push(p);return {data:lookupResult};}if(name==='driver_payment_receipt'){updates.push(p);return {data:{status:'confirmed'}};}if(name==='driver_payment_context')return {data:snapshot()};if(name==='begin_araka_payment')return {data:{...attempt,dispatch,driver_name:'Driver'}};if(name==='record_araka_status'){updates.push(p);if(p.status==='APPROVED')attempt.state='approved';return {data:attempt.state};}throw Error(name);},from:()=>({select:()=>({eq(){return this;},maybeSingle:async()=>({data:invalidCallback?null:attempt})})})};
