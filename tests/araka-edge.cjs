...
async function run({action='check',providerData={transactionId:'TX-1',statusCode:200,statusDescription:'APPROVED'},dispatch=true,sandbox=false,invalidCallback=false,transactionId='TX-1',recent=false,identity=null}={}){
 ...
 const db={auth:{getUser:async()=>({data:{user:identity},error:identity?null:{message:'invalid'}})},rpc:async(name,{p}={})=>{if(name==='issue_driver_phone_access'){updates.push(p);return {data:null};}if(name==='driver_payment_context')return {data:snapshot()};if(name==='begin_araka_payment')return {data:{...attempt,dispatch,driver_name:'Driver'}};if(name==='record_araka_status'){updates.push(p);if(p.status==='APPROVED')attempt.state='approved';return {data:attempt.state};}throw Error(name);},from:()=>({select:()=>({eq(){return this;},maybeSingle:async()=>({data:invalidCallback?null:attempt})})})};
...
(async()=>{
 assert.equal((await run({action:'phone-login'})).status,401);
 assert.equal((await run({action:'phone-login',identity:{id:'unverified',phone:'243812345678'}})).status,401);
 const phone=await run({action:'phone-login',identity:{id:'verified',phone:'243812345678',phone_confirmed_at:'2026-09-17'}});assert.equal(phone.status,200);assert.match(phone.data.token,/^[a-f0-9]{64}$/);assert.equal(phone.updates[0].user_id,'verified');assert.equal(phone.charges,0);
