...
window.GMFleetFinance = (() => {
 ...
 const cashier=()=>['admin','cashier'].includes(role());
 ...
 async function load(){
  const names=['contracts','drivers','vehicles','repayment_schedules','payments','payment_allocations','lolc_deposits','lolc_deposit_items','finance_events'];
  const data=await Promise.all(names.map(t=>ctx.rows(t,q=>t==='payment_allocations'?q.order('payment_id').order('schedule_id'):t==='lolc_deposit_items'?q.order('deposit_id').order('payment_id'):q.order('id'))));
 ...
 function payment(id){
  ...
  submit(form,'confirm_cash_payment',f=>({contract_id:f.get('contract_id'),amount:f.get('amount'),paid_on:f.get('paid_on'),method:f.get('method'),reference:f.get('reference').trim(),reason:f.get('reason').trim()}));
 }
 function deposit(){
