...
function configuration(){
 ...
 // Match the production e-commerce API used by the supplied working app.
 if(endpoint.hostname==='merchant.arakapay.com'&&['/','/auth/login','/auth/login/'].includes(endpoint.pathname)&&endpoint.protocol==='https:'&&!endpoint.username&&!endpoint.password&&!endpoint.search){
  endpoint.hostname='api.arakapay.com';endpoint.pathname='/';endpoint.hash='';
 }
 ...
 return {base,test:/uat|sandbox|staging|test/i.test(endpoint.hostname),page:paymentPageId()};
}
function paymentPageId(currency='USD'){
 const configured=Deno.env.get(currency==='CDF'?'ARAKA_PAYMENT_PAGE_ID_CDF':'ARAKA_PAYMENT_PAGE_ID');
 if(!configured)throw new Error('Page de paiement Araka non configurée pour '+currency+'.');
 const raw=configured.trim();
 const value=raw.startsWith('https://')?new URL(raw).pathname:raw;
 const id=value.replace(/\/+$/,'').split('/').pop();
 if(!id)throw new Error('Identifiant de page de paiement Araka manquant.');
 return id;
}
...
async function verify(db:any,attempt:any,force=false){
 ...
 const records=Array.isArray(lookup.data)?lookup.data:[lookup.data];
 const data=records.find((record:any)=>record&&typeof record==='object'&&(byId?record.transactionId===attempt.transaction_id:[record.originatingTransactionId,record.transactionReference].includes(attempt.reference)));
...
Deno.serve(async req=>{
 ...
 try{
  ...
  config.page=paymentPageId(context.currency);
