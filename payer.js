 ...
 function paint(data){
  ...
  if($('driverPayForm')){
   ...
   $('driverPayForm').onsubmit=async event=>{
    event.preventDefault();const f=new FormData(event.currentTarget),button=event.currentTarget.querySelector('button[type="submit"]');button.disabled=true;$('payError').textContent='';
    ...
    catch(error){try{const refreshed=await call('context');if(refreshed.attempts.some(a=>a.state==='pending'))paint(refreshed);}catch{}if($('payError'))$('payError').textContent=error.message;}finally{button.disabled=false;}
   };
  }
 }
