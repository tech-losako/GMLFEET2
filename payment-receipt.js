...
window.GMFleetReceipt = (() => {
 function create(receipt){
  ...
  function row(label,value){
   doc.setFont('helvetica','normal');doc.setFontSize(9);const lines=doc.splitTextToSize(clean(value),76);const height=Math.max(6,lines.length*4+2);
  ...
  row('Date du paiement',receipt.paid_on);row('Confirmé le',new Intl.DateTimeFormat('fr-FR',{dateStyle:'short',timeStyle:'short',timeZone:'Africa/Kinshasa'}).format(new Date(receipt.confirmed_at))+' (Kinshasa)');row('Méthode',String(receipt.method||'').replace(/Araka/gi,'Mobile Money'));
  ...
  if(receipt.lolc_amount!==undefined){row('Part LOLC',cash(receipt.lolc_amount));row('Part GML',cash(receipt.gml_amount));}
  if(receipt.covered_dates?.length){row('Échéances couvertes',receipt.covered_dates.map(d=>d.date+' : '+cash(d.amount)).join(' ; '));}
