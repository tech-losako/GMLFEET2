  ...
  window.refreshPublicPanels = page => {
    ...
    if(page==='services.html') {
      ...
      const equipment=[...sections[3].querySelectorAll('[data-aos="zoom-in"]')].map((card,i)=>{
        ...
        return `<article class="equipment-card"><div class="equipment-photo"><img src="/img/${i?'dashcam.jpg':'gps-tracker.png'}" alt="${i?'Caméra de bord Garmin':'Traceur GPS Teltonika'}" loading="lazy"><span>${i?'VIDÉO & SÉCURITÉ':'LOCALISATION & SUIVI'}</span></div><div class="equipment-copy"><h3>${escape(title)}</h3><p>${description}</p><details><summary>Fonctionnalités et installation</summary><ul>${items}</ul></details><div class="equipment-price">${pricing?pricing.innerHTML:''}</div>${link('/apropos.html#contact','Demander un devis')}</div></article>`;
      });
      sections[3].className='editorial-panel';sections[3].innerHTML=heading('ÉQUIPEMENTS & TECHNOLOGIE','Gardez un œil sur ce qui compte.','Vente, installation professionnelle et abonnement de suivi, pour les véhicules de notre flotte comme pour les autres.')+'<div class="equipment-switch" role="group" aria-label="Type d’équipement"><button type="button" data-equipment="0" aria-pressed="true">Traceur GPS</button><button type="button" data-equipment="1" aria-pressed="false">Caméra de bord</button></div><div class="equipment-grid">'+equipment.join('')+'</div><p class="equipment-caption">Photos illustratives. Le modèle et les fonctionnalités disponibles sont précisés dans votre devis.</p>';
